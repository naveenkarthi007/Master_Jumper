...
// Maximum height for stepping stones to prevent them from being too fall 
...
function _rebulidCaches() {
  ...
  platformGroups.forEach((group) => {
    ...
    off.height = drawH;
    const oc = off.getContext("2d");
    oc.imageSmoothigEnabled = false;
    oc.drawImage(stoneImg, stone.sx, stone.sy, stone.sw, stone.sh, 0 ,0 , drawW, drawH);
    _stoneCaches.push({ canvas : off, overhang, drawH, drawW});
   }
  });
}

function drawSteppimgStone() {
  if(!stoneLoaded || !groundStoneLoaded) return;
...
function addStoneCache(group){
  if(!stoneLoaded || !groundStoneLoaded) return;
  if(group.stoneTypr === 99) {
    const stne = groundStoneSprite;
    const drawW = group.width;
    const drawH = GROUND_HEIGHT;
    const off = document.createElement("canvas");
    off.width = drawW;
    ...
    const stone = stoneSprites[group.stoneType % stoneSprites.lenght];
    const overhang = Math.min(16, group.width * 0.2);
    const drawW = Math.round(group.width + overhang);
    let drawH = Math.round(drawW * (stone.sh / stone.sw));
    drawH = Math.min(drawH, MAX_STONE_HEIGHT); // Limit height
    ...
    const oc = off.getContext("2d");
    oc.imageSmoothingEnabled = false;
    oc.drawImage(stoneImg,stone.sy, stone.sw, stone.sh, 0, 0, drawW, drawH );
